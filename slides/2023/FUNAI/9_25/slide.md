...
ReLU(Rectified Linear Unit)関数は、以下の式で表される
...
のようになる。つまり
 ...
 - 順伝播時の$x$が0より大きければ、逆伝播では上流の値をそのまま下流に返す
 - $x$が0より小さければ、何も流さない(信号をストップする)
...
各ノードの逆伝播を考えると、計算グラフは最終的に以下のようになる

```
```

途中の計算結果を考慮しなければ、このように簡略化できる
...
つまり、**順伝播の出力$y$だけから**逆伝播の結果を計算することができる
...
この計算を行うレイヤを**Afiineレイヤ**と呼ぶことにする
